const ICONS: Record<string, { src: string; color?: string; hoverColor?: string }> = {
  play: { src: '/assets/icons/play.svg', color: 'var(--accent)', hoverColor: '#071018' },
  pause: { src: '/assets/icons/pause.svg', color: 'var(--accent)', hoverColor: '#071018' },
  close: { src: '/assets/icons/close.svg', color: 'var(--accent)' },
  menu: { src: '/assets/icons/menu.svg', color: 'var(--muted)' },
  settings: { src: '/assets/icons/settings.svg', color: 'var(--accent)', hoverColor: '#071018' },
  coin: { src: '/assets/icons/coin.svg', color: 'var(--accent)', hoverColor: '#071018' },
  ghost: { src: '/assets/icons/ghost.svg', color: 'var(--accent)', hoverColor: '#071018' },
  skull: { src: '/assets/icons/skull.svg', color: 'var(--accent)', hoverColor: '#071018' },
  chevron: { src: '/assets/icons/chevron.svg', color: 'var(--accent)', hoverColor: '#071018' },
  circle: { src: '/assets/icons/circle.svg', color: 'var(--accent)', hoverColor: '#071018' },
  check: { src: '/assets/icons/check.svg', color: 'currentColor' },
  refresh: { src: '/assets/icons/refresh.svg', color: 'currentColor' },
  send: { src: '/assets/icons/send.svg', color: 'currentColor' },
  copy: { src: '/assets/icons/copy.svg', color: 'currentColor' }
};

export default ICONS;
